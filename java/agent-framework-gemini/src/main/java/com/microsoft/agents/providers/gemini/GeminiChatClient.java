...
/**
 * Implements provider-neutral chat through the official Google Gen AI Java SDK.
 */
public final class GeminiChatClient implements ChatClient {
    ...
    private GeminiChatClient(GeminiChatClientOptions options, GeminiTransport transport, boolean ownsTransport) {
    ...
    public GeminiChatClientOptions options() {
        return options;
    }

    /** Returns implemented capability flags. */
    public GeminiCapabilities capabilities() {
        return GeminiCapabilities.current();
    }
    ...
    public CompletionStage<ChatResponse> completeAsync(ChatClientRequest request, RunCancellation cancellation) {
        ...
        try {
            GeminiMapper.validate(request, options);
        ...
        try {
            stage = Objects.requireNonNull(
                    ...
                    "GeminiTransport.completeAsync returned null.");
        } catch (RuntimeException exception) {
    ...
    /** Builds immutable {@link GeminiChatClient} instances. */
    public static final class Builder {
        private GeminiChatClientOptions options;
        ...
        public Builder options(GeminiChatClientOptions options) {
            this.options = Objects.requireNonNull(options, "options");
            return this;
        }

        /** Injects a caller-owned framework transport. */
        public Builder transport(GeminiTransport transport) {
            return transport(transport, false);
        }

        /** Injects a transport and optionally transfers ownership. */
        public Builder transport(GeminiTransport transport, boolean closeTransport) {
        ...
        public GeminiChatClient build() {
            GeminiChatClientOptions builtOptions = Objects.requireNonNull(options, "options");
            if (transport != null) {
                return new GeminiChatClient(builtOptions, transport, closeTransport);
            }
            return new GeminiChatClient(builtOptions, GeminiSdkTransport.create(builtOptions), true);
        }
    }
    ...
    private final class StreamingRun implements Flow.Subscriber<ChatResponseUpdate> {
        ...
        private void start() {
            ...
            try {
                Flow.Publisher<ChatResponseUpdate> publisher = Objects.requireNonNull(
                        ...
                        "GeminiTransport.completeStreaming returned null.");
        ...
        public void onError(Throwable failure) {
            fail(failure instanceof RuntimeException runtime ? runtime : GeminiChatClient.failure("stream_error"));
        }
