...
final class JdkA2AClient implements A2AClient {
    ...
    private <T> RunHandle<T> startRpc(String method, StateValue params, Function<StateValue, T> decoder) {
        ...
        requireRequestSize(body);
        HttpRequest request = postRequest(method, body, false);
        return startFinite(
                request, method, responseBody -> decoder.apply(decodeRpcResult(responseBody, id)), "application/json");
    }
    ...
    private A2AProtocolException decodeRpcError(StateValue value) {
        ...
        return new A2AProtocolException(code, message, data, null);
    }
    ...
    private void requireRequestSize(byte[] body) {
        if (body.length > options.limits().maxRequestBytes()) {
            throw new A2ATransportException("Encoded A2A request exceeds maxRequestBytes.");
        }
    }
