...
final class StrictAgentDocumentParser {
    ...
    private StrictAgentDocumentParser() {}
    ...
    static StateValue parseJson(String document) {
        ...
        try {
            ...
            if (root == null) {
                throw new DeclarativeAgentParseException("JSON agent definition is empty.");
            }
            ...
            throw new DeclarativeAgentParseException("Malformed JSON agent definition" + suffix + ".", failure);
        }
    }
            ...
            for (Event event : yaml.parse(new StringReader(document))) {
                if (event instanceof AliasEvent) {
                    throw new DeclarativeAgentParseException(
                            "YAML aliases are not allowed in declarative agent definitions.");
                }
            }
            Iterator<Object> documents = yaml.loadAll(document).iterator();
            if (!documents.hasNext()) {
                throw new DeclarativeAgentParseException("YAML agent definition is empty.");
            }
            Object root = documents.next();
            if (documents.hasNext()) {
                throw new DeclarativeAgentParseException("YAML agent definition must contain exactly one document.");
            }
            return fromYaml(root, 0);
        } catch (DeclarativeAgentParseException failure) {
            throw failure;
        } catch (YAMLException failure) {
            throw new DeclarativeAgentParseException("Malformed YAML agent definition.", failure);
        }
    }
    ...
    private static void requireDocument(String document, String format) {
        if (document == null || document.isBlank()) {
            throw new DeclarativeAgentParseException(format + " agent definition is empty.");
        }
        if (document.length() > MAX_DOCUMENT_CHARACTERS) {
            throw new DeclarativeAgentParseException(
                    format + " agent definition exceeds " + MAX_DOCUMENT_CHARACTERS + " characters.");
        }
    }
    ...
    private static StateValue fromJson(JsonNode node, int depth) {
        ...
        if (node.isNumber()) {
            ...
            if (!Double.isFinite(value.doubleValue())) {
                throw new DeclarativeAgentParseException("JSON numbers must be finite.");
            }
        ...
        throw new DeclarativeAgentParseException("Unsupported JSON token in declarative agent definition.");
    }
    ...
    private static StateValue fromYaml(Object value, int depth) {
        ...
        if (value instanceof Float || value instanceof Double) {
            ...
            if (!Double.isFinite(number)) {
                throw new DeclarativeAgentParseException("YAML numbers must be finite.");
            }
        ...
        if (value instanceof Map<?, ?> map) {
            ...
            for (Map.Entry<?, ?> entry : map.entrySet()) {
                if (!(entry.getKey() instanceof String name)) {
                    throw new DeclarativeAgentParseException("YAML object member names must be strings.");
                }
    ...
    private static void requireDepth(int depth) {
        if (depth > MAX_DEPTH) {
            throw new DeclarativeAgentParseException(
                    "Declarative agent definition exceeds maximum nesting depth " + MAX_DEPTH + ".");
        }
    }
}
