...
`agent-framework-openai` adapts the framework-owned
`com.microsoft.agents.agents.ChatClient` contract to the OpenAI Responses API. It compiles and runs
on Java 25 without preview APIs.
...
This module implements chat through the OpenAI Responses API, function tools, and the explicitly
configured image-generation tool. Embeddings, structured-output helpers, other OpenAI built-in
tools, and Azure-specific identity/service-version configuration are later work. A custom
