...
final class CosmosMemoryCursor {
    private static final String PREFIX = "afmc1.";

    private static final int MAX_LENGTH = 32_768;

    private CosmosMemoryCursor() {}
    ...
    static String decode(String partitionKey, String cursor) {
        ...
        if (cursor.isBlank() || cursor.length() > MAX_LENGTH || !cursor.startsWith(PREFIX)) {
            throw new ValidationException("Cosmos memory cursor is malformed.");
        }
        ...
        try {
            ...
            if (!partitionKey.equals(decodedPartition) || token.isBlank()) {
                throw new ValidationException("Cosmos memory cursor belongs to another tenant scope.");
            }
            return token;
