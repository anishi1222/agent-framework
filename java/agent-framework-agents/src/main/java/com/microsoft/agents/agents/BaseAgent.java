...
public abstract class BaseAgent<T> implements Agent<T> {
    ...
    public final Flow.Publisher<AgentResponseUpdate> runStreaming(
            List<Message> messages, RunOptions options, RunCancellation cancellation) {
        return runStreamingInternal(messages, options, cancellation, null);
    }
    ...
    protected final Flow.Publisher<AgentResponseUpdate> runStreamingWithSession(
        ...
        return runStreamingInternal(
                messages, options, cancellation, AgentValidation.requireNonNull(session, "session"));
    }
    ...
    private void beginStreamingRun(
        ...
        try {
            executor.execute(() -> startStreamingExecution(context, source, cancellation, sink, upstreamReference));
        } catch (RejectedExecutionException failure) {
    ...
    private void startStreamingExecution(
        ...
        source.handle().resultAsync().whenComplete((ignored, failure) -> cancellationRegistration
                .get()
                .close());
