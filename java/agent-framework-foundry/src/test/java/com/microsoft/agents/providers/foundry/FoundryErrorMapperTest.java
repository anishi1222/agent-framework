    ...
    void httpFailure_shouldRetainOnlySafeStatusIdentifiersAndServiceCode() {
        ...
        when(response.getHeaders())
                .thenReturn(new HttpHeaders()
                        ...
                        .set(HttpHeaderName.fromString("x-ms-error-code"), "forbidden"));

        RuntimeException mapped = FoundryErrorMapper.map(new HttpResponseException("body credential-secret", response));

        assertThat(mapped).isInstanceOfSatisfying(FoundryProviderException.class, failure -> {
            assertThat(failure.kind()).isEqualTo(FoundryProviderException.Kind.AUTHENTICATION);
            assertThat(failure.statusCode()).hasValue(403);
            assertThat(failure.requestId()).contains("request-1");
            assertThat(failure.correlationId()).contains("correlation-1");
            assertThat(failure.serviceCode()).contains("forbidden");
