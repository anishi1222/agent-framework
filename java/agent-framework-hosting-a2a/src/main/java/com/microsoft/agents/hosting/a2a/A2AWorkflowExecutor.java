...
public final class A2AWorkflowExecutor<I, O> implements A2AExecutor {
     ...
     */
    public A2AWorkflowExecutor(
            Workflow<I, O> workflow, List<String> outputModes, A2ALimits limits) {
    ...
    public CompletionStage<Void> executeAsync(
        ...
        return sink.updateStatusAsync(TaskState.TASK_STATE_WORKING, null)
                .thenCompose(ignored -> workflow.runAsync(
                        input, WorkflowRunOptions.defaults(), cancellation))
                .thenCompose(result -> {
                    List<Part> parts = convertOutput(result.output());
                    CompletionStage<?> artifactStage = parts.isEmpty()
                            ...
                            : sink.addArtifactAsync(
                                    ...
                                    Map.of(
                                            "workflowRunId",
                                            StateValue.string(result.runId()))));
    ...
    private I convertInput(A2AExecutionContext context) {
        ...
        if (inputType == String.class) {
            if (context.request().message().parts().stream()
                    .anyMatch(part -> !(part instanceof TextPart))) {
                throw new A2AConversionException(
                        "Workflow String input accepts only text/plain parts.");
            }
        ...
        if (inputType == Message.class) {
            return (I) A2AContentConverter.toFrameworkMessage(
                    context.request().message(), List.of("*/*"), codec);
        }
        if (inputType == StateValue.class) {
            if (context.request().message().parts().size() != 1
                    || !(context.request().message().parts().getFirst()
                            instanceof DataPart data)) {
                throw new A2AConversionException(
        ...
        if (inputType == byte[].class) {
            if (context.request().message().parts().size() != 1
                    || !(context.request().message().parts().getFirst()
                            instanceof FilePart file)
                    || !file.inline()) {
                throw new A2AConversionException(
                        "Workflow byte[] input requires exactly one inline file part.");
            }
            return (I) file.bytes();
        }
        throw new A2AConversionException(
                "Workflow input type " + inputType.getName()
                        + " has no lossless A2A mapping; use String, Message, StateValue, or byte[].");
    }

    private List<Part> convertOutput(O output) {
        ...
        if (output instanceof Message message) {
            return A2AContentConverter.toA2AParts(
                    List.of(message), outputModes, codec);
        }
        if (output instanceof byte[] bytes) {
            return List.of(FilePart.bytes(
                    bytes,
                    "workflow-output.bin",
        ...
        if (output instanceof DataContent data) {
            return List.of(FilePart.bytes(
                    data.data(), "workflow-output.bin", data.mediaType(), data.metadata()));
        }
        throw new A2AConversionException(
