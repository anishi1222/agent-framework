...
public interface A2ATaskStore {
     ...
     */
    CompletionStage<Task> updateAsync(A2APrincipal principal, Task task, TaskState expectedState);
     ...
     * @return page
     */
    CompletionStage<A2ACursorPage<Task>> listAsync(A2APrincipal principal, A2ARequests.ListTasks request);

    /**
     * Deletes one task for failure-atomic host initialization.
     *
     * @param principal authenticated principal
     * @param taskId task identifier
     * @return whether a task was removed
     */
    CompletionStage<Boolean> deleteAsync(A2APrincipal principal, String taskId);
}
